import { SignupInput } from '@rishabhpandey106/blog-common';
...
const SignupForm = () => {
  ...
  const handleSubmit = async (e:any) => {
    e.preventDefault();
    console.log(postinput.email, postinput.password, postinput.name );

    try {
      const res = await axios.post(`${DOMAIN}/api/v1/user/signup`, postinput)
  ...
  return (
    <div className="flex justify-center flex-col items-center h-screen bg-orange-200">
      <form className="bg-orange-300 p-8 rounded shadow-md w-80 lg:w-96" onSubmit={handleSubmit}>
        <h2 className="text-2xl font-bold mb-4">Create a Account</h2>
        <div className="mb-4">
      ...
      <h2 className='py-5 flex justify-center text-orange-400 text-xl font-extrabold'>Already have an account? <Link className='pl-2 underline text-orange-500' to={"/signin"}>Signin</Link> </h2>
    </div>
  );
};
